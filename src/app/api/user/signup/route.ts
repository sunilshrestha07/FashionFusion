...
export async function POST(request: Request) {
    ...
    try {
        ...
        if (user && !user.isVerified) {
            ...
            await user.save();
            await sendMail({ email, userName, verificationCode: user.verificationCode });

            return NextResponse.json(
                { success: true, message: "User updated with new verification code", user },
                { status: 200 }
            );
        }
        ...
        await newUser.save();
        await sendMail({ email, userName, verificationCode: newUser.verificationCode });
